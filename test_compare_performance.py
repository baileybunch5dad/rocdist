...
def getRMSE(predHist : np.array, predBins : np.array, actHist : np.array, actBins : np.array):
    ...
    rmse_hist, rmse_bins = np.sqrt(np.sum((predicted - actual)**2, axis = 0) / len(distances))
    return rmse, rmse_hist, rmse_bins
...
def convertDynamicDistToNumpy(dd, n_bins=100):
    ...
    dd_last_val = ddbins[-1] + ddbinsize
    dd_last_bin = ddminval + (n_bins) * ddbinsize
    print(f"{dd_last_val=}, {dd_last_bin=}")
    # Use dd_last_bin to preserve the bin size
    ddmaxval = dd_last_bin
...
def results_time_error(algorithm, bigarray: np.array, actualhist: np.array, actualbins: np.array):
    ...
    rmse, rmse_hist, rmse_bins = getRMSE(predhist, predbins, actualhist, actualbins)
    print(f'{type(algorithm).__name__} Time={elapsed:.1f} RMSE={rmse:.1f} RMSE(hist)={rmse_hist:.1f}  RMSE(bins)={rmse_bins:.1f}')
    return predhist, predbins, elapsed, rmse, rmse_hist, rmse_bins
...
def compare_to_numpy(ax, testfcn):
    ...
    dhist, dbins, dtime, derr, derr_hist, derr_bins = results_time_error(DynamicDist(), bigarray, actualhist, actualbins)
    shist, sbins, stime, serr, serr_hist, serr_bins = results_time_error(SparseDist(), bigarray, actualhist, actualbins)
    
    # Overlay the hisogram of the predictor versus the actual
    ax.stairs(actualhist, actualbins, label='numpy')
    ax.stairs(dhist, dbins, label=f'DynamicDist T={dtime:.1f}s RMSE={derr:.1f} RMSE(hist)={derr_hist:.1f}  RMSE(bins)={derr_bins:.1f}' )
    ax.stairs(shist, sbins, label=f'SparseDist  T={stime:.1f}s RMSE={serr:.1f} RMSE(hist)={serr_hist:.1f}  RMSE(bins)={serr_bins:.1f}' )
